...
def poblar():
    ...
    if mas_caro:
        print(f"   * Producto con el precio más alto: [{mas_caro.sku}] {mas_caro.nombre} (${mas_caro.precio})")

    mas_barato = Producto.objects.order_by('precio').first()
    if mas_barato:
        print(f"   * Producto con el precio más bajo: [{mas_barato.sku}] {mas_barato.nombre} (${mas_barato.precio})")
